...
## Features
- **Profile layout** – `ProfilePage.astro` renders hero, toolbox, experience grouped by company (roles stay nested under each company), writing list, and project cards sourced from `src/content/profile/*.md`.
...
- `src/content/` – `astro:content` collection files for profile entries and posts.
- `src/content.config.ts` – Astro content collections schema/loaders.
...
## Development Scripts
- Install dependencies with `pnpm install` (Astro 6.x required).
